...
import {
  ...
  reportRange,
  type Report,
} from "@/lib/gnanam/reports";

/** Format compact de l'axe des barres : 4 820 → « 4,8k ». */
...
function TopProducts({ report }: { report: Report }) {
  const max = Math.max(...report.top.map((t) => t.value));
  return (
...
export function RapportsModule() {
  ...
  const report = REPORTS[state.repPeriod];
    ...
    <div className="flex-1 overflow-y-auto px-6 pt-5 pb-28">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-[13.5px] text-[var(--gnanam-gray-600)]">{reportRange(state.repPeriod)}</div>
        </div>
      ...
      <div className="mt-4.5 grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(190px,1fr))]">
        {report.kpis.map((k) => (
          <div key={k.label} className="rounded-2xl bg-white p-4 px-4.5 shadow-[0_2px_10px_rgba(14,58,66,.05)]">
            ...
            <div className="mt-1 text-[12.5px] font-bold" style={{ color: k.trendColor }}>
              {k.trend}
            </div>
          </div>
        ))}
